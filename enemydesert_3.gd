...
const SPEED = 40.0
var left_limit: float = 1535.0  # Límite izquierdo en la posición X
var right_limit: float = 1710.0  # Límite derecho en la posición X
...
func _on_muerenemigo_body_entered(body: Node2D) -> void:
	if body.get_name() == "Finn":
		body.enemy_sound()
		queue_free()


func _on_muerefinn_body_entered(body: Node2D) -> void:
	if body.get_name() == "Finn":
