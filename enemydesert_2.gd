...
const SPEED = 40.0
var left_limit: float = 1890.0  # Límite izquierdo en la posición X
var right_limit: float = 1980.0  # Límite derecho en la posición X
...
func _on_matarenemigo_body_entered(body: Node2D) -> void:
	if body.get_name() == "Finn":
		body.enemy_sound()
		queue_free()


func _on_morirenemigo_body_entered(body: Node2D) -> void:
	if body.get_name() == "Finn":
